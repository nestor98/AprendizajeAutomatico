function [hypothesis, best_lambda] = kfoldcv_gaussianas(X, y, clases, ingenuo, k, probar_lambdas)%;(k, X, y,lambda_vals)
...
%   best_lambda = 0;
    ...
    for i = 1:size(probar_lambdas,2)
        ...
        for fold = 1:k 
            ...
            modelo = entrenarGaussianas(Xtr, ytr, clases, ingenuo, lambda);
            ...
            yhat_tr = clasificacionBayesiana(modelo, Xtr, clases);
            err_T = err_T + tasa_errores(yhat_tr, ytr); % Error en train
            
            yhat_val = clasificacionBayesiana(modelo, Xval,clases);      
    ...
    hypothesis = entrenarGaussianas(X, y, clases, ingenuo, best_lambda);
end